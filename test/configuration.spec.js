...
describe('configuration service', function () {
  ...
  it('loads interpolated values from attributes as they change', function () {
    // Arrange
    provider.setActiveInterpolation('foo', {
      prop2: true,
      prop4: true
    });
    ...
    var callbacks = [];
    attrs.$observe = jasmine.createSpy()
      .and.callFake(function (name, cb) {
        callbacks.push(cb);
      });
  ...
  it('returns the same object so calls can be chained', function () {
    expect(provider.setDefaults('foo', {}))
      .toBe(provider);
    expect(provider.setActiveInterpolation('foo', {}))
      .toBe(provider);
  });
});