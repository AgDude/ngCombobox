...
describe('autoComplete directive', function () {
  ...
  beforeEach(function () {
    ...
    deferred = $q.defer();
    eventHandlers = {};
    $scope.loadItems = jasmine.createSpy()
  ...
  function compile() {
    ...
    combobox = {
      ...
      on: jasmine.createSpy()
        .and.callFake(function (names, handler) {
          names.split(' ')
      ...
      getTags: jasmine.createSpy()
        .and.returnValue([]),
      getOptions: jasmine.createSpy()
        .and.returnValue({
  ...
  describe('basic features', function () {
    it('ensures that the suggestions list is hidden by default', function () {
      expect(isSuggestionsBoxVisible())
        .toBe(false);
    });
    ...
    it('renders all elements returned by the load function that aren\'t already added', function () {
      ...
      expect(getSuggestions()
          .length)
        .toBe(2);
      expect(getSuggestionText(0))
        .toBe('Item1');
      expect(getSuggestionText(1))
        .toBe('Item2');
    });
    ...
    it('sets the selected suggestion to null after adding it to the input field', function () {
      ...
      // Assert
      expect(suggestionList.selected)
        .toBeNull();
    });
    ...
    it('does not call the load function after adding the selected suggestion to the input field', function () {
      ...
      // Assert
      expect($scope.loadItems.calls.count())
        .toBe(1);
    });
    ...
    it('highlights the selected suggestion only', function () {
      ...
      // Assert
      expect(getSuggestion(0)
          .hasClass('selected'))
        .toBe(false);
      expect(getSuggestion(1)
          .hasClass('selected'))
        .toBe(true);
      expect(getSuggestion(2)
          .hasClass('selected'))
        .toBe(false);
    });
    ...
    it('discards all load calls but the last one', function () {
      ...
      // Assert
      expect(suggestionList.show.calls.count())
        .toBe(1);
    });
  ...
  describe('keys propagation handling', function () {
    describe('hotkeys - suggestion box is visible', function () {
      ...
      it('prevents the down arrow keydown event from being propagated', function () {
        ...
        expect(event.isDefaultPrevented())
          .toBe(true);
        expect(event.isPropagationStopped())
          .toBe(true);
      });
    ...
    describe('non-hotkeys', function () {
      it('does not prevent non-hotkeys keystrokes from being propagated', function () {
        ...
        // Assert
        expect(events[0].isDefaultPrevented())
          .toBe(false);
        expect(events[0].isPropagationStopped())
          .toBe(false);

        expect(events[1].isDefaultPrevented())
          .toBe(false);
        expect(events[1].isPropagationStopped())
          .toBe(false);

        expect(events[2].isDefaultPrevented())
          .toBe(false);
        expect(events[2].isPropagationStopped())
          .toBe(false);
      });
    });
  });
  ...
  describe('debounce-delay option', function () {
    it('initializes the option to 100 milliseconds', function () {
      ...
      // Assert
      expect(isolateScope.options.debounceDelay)
        .toBe(100);
    });
  ...
  describe('min-length option', function () {
    ...
    it('calls the load function only after the minimum amount of characters has been entered', function () {
      ...
      expect($scope.loadItems.calls.count())
        .toBe(1);
      expect($scope.loadItems.calls.argsFor(0))
        .toEqual(['ABC']);
    });
  ...
  describe('highlight-matched-text option', function () {
    it('initializes the option to true', function () {
      ...
      // Assert
      expect(isolateScope.options.highlightMatchedText)
        .toBe(true);
    });
    ...
    it('highlights the matched text in the suggestions list', function () {
      ...
      expect(getSuggestionText(1))
        .toBe('<em>a</em>b');
      expect(getSuggestionText(2))
        .toBe('b<em>a</em>');
      expect(getSuggestionText(3))
        .toBe('<em>a</em>b<em>a</em>');
      expect(getSuggestionText(4))
        .toBe('b<em>a</em>b');
    });
  ...
  describe('max-results-to-show option', function () {
    it('initializes the option to 10', function () {
      ...
      // Assert
      expect(isolateScope.options.maxResultsToShow)
        .toBe(10);
    });
